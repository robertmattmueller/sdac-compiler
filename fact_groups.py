...
def compute_groups(task, atoms, reachable_action_params):
    ...
    with timers.timing("Building translation key"):
        translation_key, atom_groups= build_translation_key(groups)
    if DEBUG:
        for group in groups:
            if len(group) >= 2:
                print("{%s}" % ", ".join(map(str, group)))

    return groups, mutex_groups, translation_key, atom_groups
