...
def parse_action(alist, type_dict, predicate_dict):
    ...
    for rest in iterator:
        ...
        if rest == ":cost":
    	    ...
    	    cost = cost.parse_cost(cost_list, outer_op, predicate_dict, parameters_dict, cost)
    	    break
    for rest in iterator:
        assert False, rest
    if eff:
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    if objects_opt[0] == ":objects":
        ...
        yield objects
        objects = dict((o.name, o) for o in objects)
        init = next(iterator)
    else:
        yield []
        init = objects_opt
    ...
    for entry in iterator:
        ...
        #if entry[0] == ":values":
        #    parse_values(entry[1:], predicate_dict, objects)
    yield use_metric
    for entry in iterator:
        assert False, entry

def parse_values(alist, predicate_dict, object_dict):
    for val in alist:
        pre, args = get_predicate(val[0], predicate_dict, object_dict)
        pre.add_value(tuple(args), (val[1], val[2]))

def get_predicate(alist, predicate_dict, object_dict):
    par = None
    pre_arg = 0
    args = []
    try:
    ...
    for index in range(0, len(alist)):
        par = object_dict[alist[index]]
        if par.type_name != pre.arguments[index].type_name:
